...
export default function SubmitAppModal({ onClose, existingTags }: Props) {
  ...
  const [categories, setCategories] = useState<Set<AppCategory>>(new Set())
  ...
  const isValid = name.trim() && url.trim() && categories.size > 0 && tags.length > 0 && why.trim() && checks.every(Boolean)
